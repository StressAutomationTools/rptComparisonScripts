...
#
# This script is intended to compare values in two single column rpt files.
# The number of elements can be different between the two files.
# An Envelope will be created
#
# use: perl RPT_Compare_els.pl old.rpt new.rpt [groups.ses]
#
# old.rpt:
...
sub readRPT {
	...
	while(<RPT>){
		if(m/^\s+(\d+)\s+(\S+)(\s*)$/){
			...
			if($rptResults{$EID}[0] > $value){
				$rptResults{$EID}[0] = $value;
			}
			if($rptResults{$EID}[1] < $value){
				$rptResults{$EID}[1] = $value;
			}
		}
	}
	close(RPT);
